...
if __name__ == "__main__":
    ...
    # Usage (help)
    usage   = "python modelviz.py appname1 appname2 ... [options]"
    for s in ("help", "-h", "--help"):
        if s in args:
            print(usage)
    # Splitting args in appnames and options
    apps    = []
    options = {}
    while len(args) > 0:
        if args[0].startswith('-'):
            options[args[0].strip('-')] = args[1]
            args = args[2:]
        else:
            apps.append(args.pop())
    # Generating dot file
    print(generate_dot(apps, **options))
